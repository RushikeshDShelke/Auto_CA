    ...
    public function execute()
    {
        ...
        if ($data) {
			...
			try{
				...
				$result = $uploader->save($mediaDirectory->getAbsolutePath('advancecontacts_advancecontact'));
					if($result['error']==0)
					{
						$data['image'] = 'advancecontacts_advancecontact' . $result['file'];
					}
			} catch (\Exception $e) {
				//unset($data['image']);
            }
            ...
            try {
                $model->save();
                $this->messageManager->addSuccess(__('The Advancecontact has been saved.'));
