...
class Shipment
{
  ...
  protected function _generateBarcode($text) {
    ...
    $t1 = $resource->getTableName('sales_shipment_grid');
    $tableName = $resource->getTableName('sales_order');

    $check = substr($text,15,10000);
    ...
    $config = new \Zend_Config([
      'barcode' => 'code128',
      'barcodeParams' => [
        'text' => $this->_extractShipmentNumber($text),
        'drawText' => true
      ],
      'renderer' => 'image',
   ...
   * @return string
   */
  protected function _extractShipmentNumber($text) {
