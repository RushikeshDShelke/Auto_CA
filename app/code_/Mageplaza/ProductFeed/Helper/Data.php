    ...
    public function getProductsData($feed, $productAttributes = [], $productIds = [])
    {
        ...
        foreach ($productCollection as $product) {
            ...
            if($product->getTypeId() == 'simple'){
                ...
                $sourceItems = $objectManager->get('Magento\InventoryApi\Api\GetSourceItemsBySkuInterface')->execute($oriProduct->getSku());
                $stockStatus = false;
		...
		if($qty)
		{
			$salable_qty            = $qty[0]['qty'];
                	if($salable_qty)$stockStatus = $product->setData('quantity_and_stock_status', 'in stock');
                	else $stockStatus = $product->setData('quantity_and_stock_status', 'out of stock');
		}
		//$salable_qty            = $qty[0]['qty'];
		//if($salable_qty)$stockStatus = $product->setData('quantity_and_stock_status', 'in stock');
		//else $stockStatus = $product->setData('quantity_and_stock_status', 'out of stock');
