...
class Manualtrack extends \Magento\Sales\Controller\Adminhtml\Order
{
    /**
     ...
     * @see _isAllowed()
     */
    const ADMIN_RESOURCE = 'Magento_Sales::manualtrack';
    ...
    public function execute()
    {
        ...
        $tablename_query = $resource->getTableName('wk_secondgrid_records'); 
        $query_sql = "Select * FROM " . $tablename_query." Where `payment_method` LIKE '$mode%' ORDER BY `entity_id` ASC LIMIT 1";
        $result_sql = $connection->fetchRow($query_sql);
        
        if($result_sql)
        {
        
            $manual_doc = $result_sql['docket_no'];
        }
        else
        {
            $manual_doc = '';
        ...
        if(($conf == '1') && ($check_valid == ''))
        {
            ...
            if($manual_doc == '')
        {
            $manualReasonr = 'Manual Tracking number are not available,please upload docket number';
            $this->messageManager->addError(__('Warning Reason:     '.$manualReasonr));  
        }
        ...
        else
        {
              ...
              $fieldsdemo = array (
                            ...
                            {  
                                 ...
                                 $delete_sql = "Delete FROM " . $tablename_query ." Where `docket_no` = '$docketnum'";
                                 ...
                                 if ($order->canShip()) 
                                 {
                                    ...
                                    foreach ($order->getAllItems() AS $orderItem) 
                                    {
                                        ...
                                        $ordert->addStatusHistoryComment(
        ...
        if($manual_doc == '')
        {
            $manualReason = 'Manual Tracking number are not available,please upload docket number';
            $this->messageManager->addError(__('Warning Reason:     '.$manualReason));  
        }
     ...
     {        
        $cid = $objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface')->getValue('prshippingtracking/services/shippingmodule/customer_id');    
                     ...
                     if($docketnum1 != NULL)
                     {
                        ...
                        $delete_sql = "Delete FROM " . $tablename_query ." Where `docket_no` = '$docketnum1'";
                        $connection->query($delete_sql);
