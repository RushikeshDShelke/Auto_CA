...
class Custinvcshipgripcolumn extends Column
{
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as & $item) {
                $order = $this->orderFactory->create()->loadByIncrementId($item["order_increment_id"]);
					...
					$custshivgridcolumn = $invoice->getIncrementId();// invoice increment id
					$objectManager = \Magento\Framework\App\ObjectManager::getInstance(); 
			$objectManager  =   \Magento\Framework\App\ObjectManager::getInstance();
			$cdat = $objectManager->create('Kellton\Custominvoice\Model\ResourceModel\Customcinvoice\Collection');
			$cdat->addFieldToFilter('invoice_number', array('eq' => $custshivgridcolumn));
					foreach($cdat as $invicedata){
						$item['custshivgridcolumn'] = $invicedata['custom_invoice_number'];
					}				
					}
            }
        }
