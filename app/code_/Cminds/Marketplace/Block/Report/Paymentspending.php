...
class Paymentspending extends Template
{
    ...
    public function getPaymentPendingCollection() 
    {
        ...
        $intEntityName = $this->_resourceConnection->getTableName('catalog_product_entity_int');
        $orderItemTable = $this->_resourceConnection->getTableName('sales_order_item');

        $collection = $this->_salesOrder->getCollection();
        $collection->addAttributeToFilter('main_table.status', array('in' => array("pending_payment","Pending Payment")));

        $collection->getSelect()
            ->joinInner(array('i' => $orderItemTable), 'i.order_id = main_table.entity_id')
