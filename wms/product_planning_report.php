...
?>
<!--<form action="product_planning_report.php" name="inventory-report-filter" method="post">
...
if($collection)
{
        echo "<table><th>Craft</th><th>Product Name</th><th>SKU</th><th>Artisan Name</th><th>Qty in CM</th><th>Qty in MC</th><th>Cost Without GST</th><th>Cost with GST</th><th>Retail Price without GST</th><th>Retail Price with GST</th>";
        foreach($collection as $item)
        {
		...
		$no_of_days     = round($diff / 86400); */
                ...
                foreach($AllInventoryBySources as $sourceInventory)
                {
                        ...
                        {
                                $CM_WH = $sourceInventory['quantity'];
			}
			else{
				$MC_WH =  $MC_WH + $sourceInventory['quantity'];
			}
		}
		/*
		...
		if (array_key_exists($productObj->getTaxClassId(),$taxValueArray))
		{
			$productTaxPercentage = $taxValueArray[$productObj->getTaxClassId()];
		}
		$ArtisanTaxPercentage = 0;
                if (array_key_exists($productObj->getArtisanGst(),$ArtisantaxValueArray))
                {
                        $ArtisanTaxPercentage = $ArtisantaxValueArray[$productObj->getArtisanGst()];
		}
		$optionId = $productObj->getCreatorId();
		$attribute = $productObj->getResource()->getAttribute('creator_id');
		$supplierName = '';
		if ($attribute->usesSource()) {
    			$supplierName = $attribute->getSource()->getOptionText($optionId);
		}
		//echo $productTaxPercentage;
		//echo  $ArtisanTaxPercentage;
		 //die;
                $mc_price_without_gst   = (int)$productObj->getMarketplaceFee(); //Invoice Unit Price without GST
                $gst_value              = ((int)$productObj->getMarketplaceFee()*(int)$ArtisanTaxPercentage)/100; //Invoice GST value of Unit price
		$mc_price_with_gst      = ($mc_price_without_gst + $gst_value); //Invoice Unit Price with GST
		$price_without_gst   = (int)$productObj->getPrice(); //Invoice Unit Price without GST
                $gst_value              = ((int)$productObj->getPrice()*(int)$productTaxPercentage)/100; //Invoice GST value of Unit price
		$price_with_gst      = ($price_without_gst + $gst_value); //Invoice Unit Price with GST
		...
		if($categoryIds)
		{
			...
			$categoryName = $category->getName();
                ...
                echo "<tr><td>".$categoryName."</td><td>".$productObj->getName()."</td><td>".$productObj->getSku()."</td><td>".$supplierName."</td><td>".$CM_WH."</td><td>".$MC_WH."</td><td>".$productObj->getMarketplaceFee()."</td><td>".$mc_price_with_gst."</td><td>".$price_without_gst."</td><td>".$price_with_gst."</td>";
                if($_SESSION['role'] != 'VIEWER')
