...
if($_REQUEST)
{
	...
	//$mail->addTo(['cm.techlead@craftmaestros.com'], 'Craft Maestros');
        $mail->setSubject("Review PO No: CM".date("y")."00".$lastInsertedId);
        $mail->send();
	...
	{
		if($key != 'supplier_id' && $key != 'payment_term' && $key != 'freight_term' && $key!='preview' && $key != 'payment_term_other' &&  $key != 'maillink' &&  $key != 'id')
		{
			if($value)
			{
