...
if(isset($_REQUEST['supplier_id']) && !empty($_REQUEST['supplier_id']))
{
	...
	->addAttributeToFilter('creator_id', trim($_REQUEST['supplier_id']))
	->addAttributeToFilter('type_id', \Magento\Catalog\Model\Product\Type::TYPE_SIMPLE)
        ->load();
	$customer = $objectManager->create('Magento\Customer\Model\Customer')->load(trim($_REQUEST['supplier_id']));
...
//echo $customer->getLastname(); //Print Customer Last Name
        ...
        <form id="PO_form_data" name="PO_form_data"  action="wms_PO_orderDataEdit.php">
	<h2>Purchase Order Form of <?php echo $customer->getFirstname()." ".$customer->getLastname() ?></h2>
	<input type="hidden" id="supplier_id" name="supplier_id" value="<?php echo $_REQUEST['supplier_id'] ?>">
...
<?php
        ...
        echo "</table><input type='hidden' name='supplier_id' value='".trim($_REQUEST['supplier_id'])."'><input type='hidden' name='po_no' value ='".$_REQUEST['po_no']."'>";
